...
export const TRIAGE_DEFS = {
  'urgent-individual': { label: 'Urgent Individual Case', color: 'var(--red)', bg: 'var(--red-d)', order: 0 },
  'new-pattern': { label: 'New Pattern Emerging', color: 'var(--orange)', bg: 'var(--orange-d)', order: 1 },
  'needs-review': { label: 'Needs Human Review', color: 'var(--blue)', bg: 'var(--blue-d)', order: 2 },
  duplicate: { label: 'Possible Duplicate / Noise', color: 'var(--tx2)', bg: 'var(--bg3)', order: 3 },
  connected: { label: 'Connected to Known Problem', color: 'var(--acc)', bg: 'var(--acc-d)', order: 4 },
  'not-a-concern': { label: 'Not a Concern', color: 'var(--green)', bg: 'var(--green-d)', order: 5 },
};
...
export function triageInfo(c, problems) {
  ...
  return { cat, label: def.label, color: def.color, bg: def.bg, order: def.order, note };
}
...
// Business-lifecycle statuses a Department Manager can pick by hand — the
// earlier two (new, assigned-*) are set automatically as a side effect of
// the assignment actions instead. Deliberately non-technical: the actual
// engineering work happens in Jira/vendor systems (see the External Ticket
// field), this just tracks where the concern stands for the customer.
