...
export async function loginAction(prevState, formData) {
  ...
  if (!role) {
    return { error: 'No account found for that email. Pick one of the demo accounts below.' };
  }
  if (password !== DEMO_PASSWORD) {
    return { error: 'Incorrect password. The demo password is shown below the form.' };
  }
